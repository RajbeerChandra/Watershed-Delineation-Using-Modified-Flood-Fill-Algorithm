	...
	for (int i = 0; i < rows; ++i)
	{
		for (int j = 0; j < cols; ++j)
		{
			...
			if (value == -1)
				...
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 0, 255); // Red
			else if (value == 2)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 255, 0); // Green
			else if (value == 4)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(255, 0, 0); // Blue
			else if (value == 8)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 255, 255); // Yellow
			else if (value == 16)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 128, 255); // Orange
			else if (value == 32)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(255, 0, 255); // Purple
			else if (value == 64)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(255, 128, 0); // Pink
			else if (value == 128)
				image.at<cv::Vec3b>(i, j) = cv::Vec3b(255, 255, 255); // White
		}
	}
...
int main(int argc, char **argv)
{
	...
	else if (n == 2)
	{
		...
		cout << "run the program again ! wrong key pressed." << endl;
